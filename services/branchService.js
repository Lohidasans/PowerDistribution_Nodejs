...
const createBranch = async (req, res) => {
  try {
    ...
    if (!branch_no || !branch_name) {
      return commonService.badRequest(res, "Branch No and Branch Name are required");
    }
...
const listBranches = async (req, res) => {
  try {
    ...
    if (searchKey) {
      where[Op.or] = [
        { branch_no: { [Op.iLike]: `%${searchKey}%` } },
        { branch_name: { [Op.iLike]: `%${searchKey}%` } },
        { contact_person: { [Op.iLike]: `%${searchKey}%` } },
...
const updateBranch = async (req, res) => {
    try {
        const { id } = req.params;
        const branch = await common.findById(models.Branch, id, res);
        if (!branch) return;
        ...
        if (branch_no && branch_no !== branch.branch_no) {
            ...
            if (exists) return common.badRequest(res, "Branch No already exists");
        }

        await branch.update(req.body);
        return common.ok(res, { branch });
    } catch (err) {
