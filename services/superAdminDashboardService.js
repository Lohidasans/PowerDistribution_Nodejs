...
/* =========================================================
   ...
   Supports: period (today|week|month|year) OR from_date + to_date
   Also supports branch_id filter.
========================================================= */
const buildDateCondition = ({ period, from_date, to_date }, column, replacements, suffix = '') => {
    ...
    let fromKey = `from_date${suffix}`;
    let toKey = `to_date${suffix}`;
...
const getSuperAdminDashboard = async (req, res) => {
    try {
        ...
        // ── Build reusable date condition for each table alias ────
        const makeRepl = () => ({});

        // ============================================================
        // 1. SALES KPI – 4 period cards
        // ============================================================
        const buildSalesKPI = (labelPeriod, rep) => {
            ...
            if (labelPeriod === 'today') {
                ...
                dateClause = `AND DATE(sib.invoice_date) BETWEEN :s_from AND :s_to`;
            ...
            return `
                SELECT
                    COUNT(sib.id)::int                           AS bill_count,
                    COALESCE(SUM(sib.total_quantity), 0)         AS total_quantity,
                    COALESCE(SUM(sibi.gross_weight), 0)          AS total_gross_weight,
                    COALESCE(SUM(sibi.net_weight), 0)            AS total_net_weight,
                    COALESCE(SUM(sib.net_total), 0)              AS net_total,
                    COALESCE(SUM(sib.total_amount), 0)           AS total_amount
                FROM sales_invoice_bills sib
                LEFT JOIN sales_invoice_bill_items sibi
        ...
        const skBranch = branch_id ? `AND g.branch_id IS NOT NULL` : ''; // GRNs don't have branch_id; use products
        ...
        //    Fully sold-out: SUM(productItemDetails.quantity) = 0
        //    profit = selling_price (sales_invoice_bill_items.amount) – purchase_cost (productGrnInfos.total_amount)
        // ============================================================
        const profitRep = {};
        const profitBranch = branch_id ? `AND sib.branch_id = :profit_branch` : '';
        if (branch_id) profitRep.profit_branch = branch_id;
        const profitDateCond = buildDateCondition(dateOpts, 'sib.invoice_date', profitRep, '_profit');

        const profitKpiQuery = `
            WITH sold_out_products AS (
                -- Products where total remaining quantity = 0 (fully sold out)
        ...
        // ============================================================
        // 5. AMOUNT COLLECTION – Pie chart (payments table)
        // ============================================================
        const collRep = {};
        ...
        // ============================================================
        const jewRep = {};
        const jewDateCond = buildDateCondition(dateOpts, 'sr.return_date', jewRep, '_jew');
        ...
        // ============================================================
        const todayRep = {}; const monthRep = {}; const ytdRep = {}; const totalRep = {};
        ...
        const [
            ...
            sequelize.query(buildSalesKPI('today', todayRep), { replacements: todayRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(buildSalesKPI('month', monthRep), { replacements: monthRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(buildSalesKPI('ytd', ytdRep), { replacements: ytdRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(buildSalesKPI('total', totalRep), { replacements: totalRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(grnPurchaseQuery, { replacements: stockKpiRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(totalStockQuery, { replacements: stockKpiRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(grnUpdatedQuery, { replacements: stockKpiRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(grnPendingQuery, { replacements: stockKpiRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(statisticsQuery, { replacements: statsRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(profitKpiQuery, { replacements: profitRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(amountCollectionQuery, { replacements: collRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(salesReturnQuery, { replacements: jewRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(oldJewelQuery, { replacements: ojRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(jewelRepairQuery, { replacements: jrRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(deliveryChallanQuery, { replacements: dcRep, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(vendorOverviewQuery, { replacements: {}, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(vendorPaidQuery, { replacements: {}, type: sequelize.QueryTypes.SELECT }),
            sequelize.query(purchaseVsSalesQuery, { replacements: pvRep, type: sequelize.QueryTypes.SELECT }),
        ]);
